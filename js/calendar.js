...
class CalendarManager {
    ...
    loadMeetingRooms() {
        // 전역 회의실 목록 사용 (MEETING_ROOMS가 정의되어 있으면 사용, 없으면 스토리지에서 로드)
        if (typeof MEETING_ROOMS !== 'undefined') {
            this.meetingRooms = MEETING_ROOMS;
        } else {
            // 스토리지에서 로드하거나 기본값 사용
        ...
        // 초기 데이터가 없으면 저장
        if (!storageManager.getData('meeting_rooms') && this.meetingRooms.length > 0) {
    ...
    updateTitle() {
        ...
        switch(this.currentView) {
            ...
            case 'day':
                const weekdays = ['일', '월', '화', '수', '목', '금', '토'];
                const dayOfWeek = weekdays[this.currentDate.getDay()];
                const dayNum = String(this.currentDate.getDate()).padStart(2, '0');
                const monthNum = String(month).padStart(2, '0');
                titleElement.textContent = `${year}년 ${monthNum}월 ${dayNum}일(${dayOfWeek})`;
                break;
        }
    }
    ...
    showAddScheduleModal(presetDate = null, presetTime = null) {
        ...
        if (form) {
            ...
            if (presetTime) {
                ...
                if (startHourSelect) startHourSelect.value = paddedHour;
    ...
    setupLocationHandler() {
        ...
        const locationCustom = document.getElementById('scheduleLocationCustom');

        if (!locationSelect || !locationCustom) return;
    ...
    saveSchedule() {
        ...
        if (locationCustom && locationCustom.style.display !== 'none' && locationCustom.value.trim()) {
            // 직접 입력한 경우
            location = locationCustom.value.trim();
                    ...
                    <form id="scheduleForm" class="schedule-form">
                        ...
                        <div class="form-group">
                            ...
                            <div style="display: flex; gap: 8px; align-items: center;">
                                <select id="scheduleLocation" name="location" style="flex: 1;">
                                    <option value="">직접 입력</option>
                                    <optgroup label="회의실">
                                        ${typeof MEETING_ROOM_NAMES !== 'undefined' ? 
                                            MEETING_ROOM_NAMES.map(room => `<option value="${room}">${room}</option>`).join('') :
                                            ''}
                                    </optgroup>
                                    <optgroup label="기타 장소">
                                        <option value="화상회의">화상회의</option>
                                        <option value="외부">외부</option>
                                        <option value="카페테리아">카페테리아</option>
                                        <option value="라운지">라운지</option>
                                    </optgroup>
                                </select>
                                <input type="text" id="scheduleLocationCustom" name="locationCustom" 
                                       placeholder="직접 입력" 
